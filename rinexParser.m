function [XYZ_station,obs,observablesHeader,measurementsInterval]=rinexParser()
...
filePath = 'D:\Studia\Semestr 4\Satka\Maciek\Projekt2\ALAC00ESP_R_20170610000_01D_EN.rnx';
%%% ------ Input--- %%%%%%%%%%
%
...
%
% This functions uses functions str2double for quick conversion from
...
while (true)
    ...
    if strfind(line,'APPROX POSITION XYZ')                                  % Receiver aprox position
        XYZ_station=real(str2double(line(1:60)));
...
while(~feof(idFile))                                                        % Until end of file
    ...
    for i = 1:real(currentSatellites)                                       % Read the epoch satellites
        ...
        for m = 1:nObservables % Number of observables in the line (Generally 3)
            ...
            measurementsValue(m) = str2double(value);                      % convert string to double
        end
